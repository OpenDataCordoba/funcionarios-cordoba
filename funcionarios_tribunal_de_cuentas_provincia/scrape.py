...
def obtener_nombre_archivos_historicos():
    ...
    archivos_historicos = []
    root_archivos = os.path.join(".","data")

    for root, dirs, files in os.walk(root_archivos):
        for fname in files:
...
def obtener_ultimo_relevamiento_cargos(listado_archivos):
    ...
    path_archivo = os.path.join(".","data", ultimo_relevamiento.date().isoformat() + ".csv")
...
if __name__ == "__main__":
    ...
    df_funcionarios_actuales.to_csv(os.path.join('.','data', datetime.date.today().isoformat() + '.csv'), index=False)
